...
public class EditTaxPopup extends Sidebar {
     ...
     * Constructor
     */
    public EditTaxPopup(WebDriver driver) {
        this.driver = driver;
    ...
    public boolean isDisplayRemoveButton() {
        boolean isDisplay = false;
        try {
            isDisplay = REMOVE_BUTTON.isDisplayed();
        } catch (Exception e) {
            Log.addLog("css of REMOVE_BUTTON");
        }
        return isDisplay;
    }
    ...
    public boolean isDisplay() {
        return isDisplayPopupTitle() && getPopupTitle().toLowerCase().contains("sửa thuế"); //figma dể chi tiết thuế, sẽ check trên real UI sau
